...
for problem_type in [
    ...
    for i_rep in range(n_rep):
        for kwargs in [
            ...
            plt.savefig(f"plot_hist/tumor2d/{problem.get_id()}_{i_rep}_1d_kdes_integrated.png")

            slad.plot_cis(problem, distance_names, data_dir=data_dir, problem_suff=f"_{i_rep}")
            plt.savefig(f"plot_hist/tumor2d/{problem.get_id()}_{i_rep}_cis.png")

            slad.plot_1d_kdes(problem, distance_names, data_dir=data_dir, problem_suff=f"_{i_rep}")
            plt.savefig(f"plot_hist/tumor2d/{problem.get_id()}_{i_rep}_1d_kdes.png")
