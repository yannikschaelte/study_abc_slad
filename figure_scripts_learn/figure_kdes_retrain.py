...
def plot_kdes(problem_type, axes):
    ...
    for i_dist, dist in enumerate(distance_names):
        for i_retrain, retrain in enumerate(retrains):
            retrain_suf = ""
            if retrain:
                retrain_suf = "__Retrain"
            h = pyabc.History(
                f"sqlite:///data_learn_pilot/{problem.get_id()}_{i_rep}/db_frac_{frac}_{dist}{retrain_suf}.db",
                create=False)
            for i_par, par in enumerate(gt_par.keys()):
                ...
                pyabc.visualization.plot_kde_1d_highlevel(
                    ...
                    label=f"{dist} - Retrain: {retrain}%",
                    alpha=(i_retrain+1)/len(retrains),
                )
...
for fmt in ["pdf", "png"]:
    plt.savefig(f"figures_learn/figure_kdes_retrain.{fmt}", format=fmt, dpi=200)
